    ...
    >
      <div className="relative aspect-square overflow-hidden rounded-2xl bg-stone-100 dark:bg-slate-900">
        <Link to={`/product/${product.id}`} className="block h-full w-full">
          {/* Discount Badge */}
          {product.discount && (
            <div className="absolute left-3 top-3 z-10 rounded-full bg-emerald-600 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white md:left-4 md:top-4 md:text-xs">
        ...
        <button 
          ...
          className={`absolute right-4 top-4 z-10 rounded-full p-2.5 transition-all shadow-sm md:right-4 md:top-4 ${
            isWishlisted 
        ...
        <button 
          ...
          className="absolute bottom-4 right-4 rounded-full bg-white p-3.5 text-stone-900 shadow-xl transition-all md:bottom-4 md:right-4 md:translate-y-10 md:p-3 md:opacity-0 md:group-hover:translate-y-0 md:group-hover:opacity-100 hover:bg-emerald-600 hover:text-white active:scale-90 dark:bg-slate-950 dark:text-white dark:hover:bg-emerald-600"
        >
      ...
      <div className="space-y-1 px-1">
        <Link to={`/product/${product.id}`} className="block">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex shrink-0 items-center gap-1 text-[10px] text-amber-500 md:text-sm">
              <Star className="h-3 w-3 fill-current md:h-4 md:w-4" />
              <span>{product.rating || '4.9'}</span>
            </div>
          </div>
          <p className="text-[10px] text-stone-500 md:text-sm dark:text-slate-400">{product.category}</p>
        </Link>
        <div className="flex items-center gap-2 mt-1 md:mt-2">
          ...
          {product.discount && (
            <p className="text-[10px] text-stone-400 line-through md:text-sm dark:text-slate-500">
