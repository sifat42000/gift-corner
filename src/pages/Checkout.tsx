  ...
  if (cart.length === 0 && !isOrdered) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center px-4 text-center">
        ...
        <p className="mb-8 text-stone-500">Add some items to your cart to proceed to checkout.</p>
        <Link to="/products" className="rounded-full bg-emerald-600 px-8 py-4 font-bold text-white">
          Browse Products
        </Link>
      </div>
    );
  ...
  if (isOrdered) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center">
        ...
        <h1 className="mb-4 text-3xl font-bold text-stone-900 md:text-4xl">Order Placed Successfully!</h1>
        <p className="mb-8 text-stone-500">
          Thank you for your purchase. We've sent a confirmation email to your inbox. 
          Your order ID is <span className="font-bold text-stone-900">#GC-{Math.floor(Math.random() * 100000)}</span>
        </p>
        <div className="flex flex-col gap-4 sm:flex-row justify-center">
          <Link to="/" className="rounded-full bg-stone-900 px-8 py-4 font-bold text-white transition-all hover:bg-stone-800">
            Back to Home
          </Link>
          <Link to="/products" className="rounded-full border border-stone-200 px-8 py-4 font-bold text-stone-900 transition-all hover:bg-stone-50">
            Continue Shopping
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8">
        <button 
          onClick={() => navigate(-1)}
          className="mb-4 flex items-center gap-2 text-sm font-bold text-stone-500 hover:text-stone-900"
        >
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-12 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-8">
          {/* Shipping Address */}
          <section className="rounded-3xl border border-stone-100 bg-white p-6 md:p-8">
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full rounded-xl border border-stone-200 bg-stone-50 py-3 pl-11 pr-4 text-sm focus:border-emerald-600 focus:bg-white focus:outline-none"
                  />
                </div>
              </div>
              ...
              <div className="md:col-span-2 space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-stone-400">Full Address</label>
                <textarea 
                  required
                  name="address"
                  ...
                  rows={3}
                  className="w-full rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm focus:border-emerald-600 focus:bg-white focus:outline-none"
                />
              </div>
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-stone-400">Area / Zip Code</label>
                <input 
                  required
                  type="text"
                  name="area"
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="font-bold text-stone-900">Inside Dhaka</span>
                  <span className="text-sm font-bold text-emerald-600">৳60</span>
                </div>
                <p className="text-xs text-stone-500">Delivery within 24-48 hours</p>
              </button>
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="font-bold text-stone-900">Outside Dhaka</span>
                  <span className="text-sm font-bold text-emerald-600">৳120</span>
                </div>
                <p className="text-xs text-stone-500">Delivery within 3-5 days</p>
              </button>
            </div>
          </section>
          ...
          <section className="rounded-3xl border border-stone-100 bg-white p-6 md:p-8">
            ...
            <div className="space-y-4">
              ...
              <label className={`flex cursor-pointer items-center gap-4 rounded-2xl border-2 p-4 transition-all ${
                formData.paymentMethod === 'cod' ? 'border-emerald-600 bg-emerald-50' : 'border-stone-100 hover:border-stone-200'
              }`}>
                <input 
                  type="radio" 
                  name="paymentMethod" 
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-stone-500">Pay when you receive the gift</p>
                </div>
              </label>

              {/* bKash */}
              <div className={`rounded-2xl border-2 transition-all ${
                formData.paymentMethod === 'bkash' ? 'border-[#D12053] bg-pink-50/30' : 'border-stone-100 hover:border-stone-200'
              }`}>
                <label className="flex cursor-pointer items-center gap-4 p-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <p className="font-bold text-stone-900">bKash Send Money</p>
                      <span className="rounded bg-[#D12053] px-1.5 py-0.5 text-[10px] font-bold text-white uppercase">Fast</span>
                    </div>
                    <p className="text-xs text-stone-500">Pay upfront via bKash</p>
                  </div>
                ...
                <AnimatePresence>
                  {formData.paymentMethod === 'bkash' && (
                    ...
                    >
                      <div className="rounded-xl bg-white p-4 border border-pink-100 space-y-3">
                        <div className="flex items-start gap-2 text-xs text-stone-600">
                          ...
                          <div className="space-y-1">
                            ...
                            <ol className="list-decimal pl-4 space-y-1">
                              ...
                              <li>Enter Amount: <span className="font-bold text-emerald-600">৳{finalTotal.toFixed(0)}</span></li>
                        ...
                        <div className="space-y-2 pt-2">
                          <label className="text-[10px] font-bold uppercase tracking-widest text-stone-400">Transaction ID</label>
                          <div className="relative">
                            <Smartphone className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-stone-400" />
                            <input 
                              ...
                              className="w-full rounded-lg border border-stone-200 bg-stone-50 py-2.5 pl-10 pr-4 text-sm focus:border-[#D12053] focus:outline-none"
                            />
                          </div>
                        </div>
                      </div>
        ...
        <div className="space-y-6">
          <div className="sticky top-24 rounded-3xl bg-stone-50 p-6 md:p-8">
            ...
            <div className="mb-6 max-h-60 overflow-y-auto pr-2 space-y-4 no-scrollbar">
              {cart.map((item) => (
                <div key={item.id} className="flex items-center gap-3">
                  <div className="h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-white border border-stone-100">
            ...
            <div className="space-y-4 border-t border-stone-200 pt-6">
              ...
              <div className="flex justify-between text-lg font-bold">
                ...
                <div className="text-right">
                  <span className="block text-emerald-600">${finalTotal.toFixed(2)}</span>
                  <span className="text-[10px] font-medium text-stone-400">Approx. ৳{(finalTotal * 115).toFixed(0)}</span>
                </div>
              </div>
            </div>
            ...
            <button 
              type="submit"
              disabled={loading}
              className="mt-8 w-full rounded-2xl bg-emerald-600 py-4 text-sm font-bold text-white shadow-lg shadow-emerald-200 transition-all hover:bg-emerald-700 hover:shadow-none active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
